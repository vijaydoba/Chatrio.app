...
10. ✅ **Closed testing track ("Closed testing - Alpha") configured 2026-08-04** — all 177 available countries/regions targeted, release "1 (1.0)" created by reusing the existing signed AAB from internal testing (no rebuild needed). Tester email list ("Testers") currently has **3 of the required 12**: `vijay83061@gmail.com`, `loserbuddy070@gmail.com`, `boradparth0@gmail.com`. **Still need 9 more real testers** — each needs their own Google account and their own physical device (see the account/device-fraud-detection note in project memory `mobile-app-circles-android`); this cannot be gamed with multiple accounts on one device.
...
12. ✅ **Closed testing release submitted to Google for review 2026-08-04** — 14 pending changes (release, country targeting, category, store contact, child safety declaration) sent together via Publishing overview. Google's estimate: "typically completed within seven days but may take longer." **The closed-testing opt-in link will not function until this review is approved** — until then there is nothing further to configure on the Play Console side.
13. Success metric before investing further: does 7-day retention among app
...
- ✅ **Closed testing track set up and submitted for Google review (2026-08-04)** — countries targeted, release created, Child Safety Standards page (`/child-safety`) written and deployed, declaration completed, 14 changes submitted together. See §6 items 9-12 for the full breakdown.
- ⏳ **Waiting on Google's review** (up to ~7 days per Google's estimate) — the closed-testing opt-in link won't work until this is approved. Nothing further to do in Play Console until then.
- ⏸ **Next**: production access requires a **closed test with 12+ testers for 14+ days** first (per Play Console's Production panel) — internal testing has no such gate, but promoting beyond it does. Tester list currently has **3 of 12** (see §6 item 10); still need 9 more real people, each on their own device, to open the opt-in link and install once Google's review clears.
